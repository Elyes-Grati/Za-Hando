# Za Hando — Natural Language Control of a 4-DOF Robotic Arm Using Local LLM Planning, Computer Vision, and Analytical Inverse Kinematics
...
Field of Study: Industrial IT and Automation Engineering (IIA)
...
Za Hando addresses all five of these challenges through the layered pipeline described in this document.
...
```bash
python -m venv za-hando-env

# Activate on Linux / macOS
source za-hando-env/bin/activate

# Activate on Windows (Command Prompt)
za-hando-env\Scripts\activate.bat

# Activate on Windows (PowerShell)
za-hando-env\Scripts\Activate.ps1
```
...
Only the mechanical model files (STL/CAD) were used from this repository. No code, firmware, or software from that project is present in Za Hando. All control software — the Arduino firmware, the calibration tool, the vision pipeline, the IK solver, the LLM interface, and the execution loop — was designed and implemented independently by the Za Hando team.
